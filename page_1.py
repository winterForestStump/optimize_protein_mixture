...
with st.expander("Краткое описание", expanded=False):
    st.write("Это веб-приложение рассчитывает самую дешевую рецептуру комбикорма (смеси) для достижения заданного содержания протеина. Оно находит оптимальное сочетание до четырех различных ингредиентов, а также позволяет учесть добавление карбамида для повышения уровня белка. Цель — минимизировать себестоимость тонны смеси или рассчитать прибыль при известной цене продажи.")
with st.expander("Инструкция по использованию", expanded=False):
    st.write('''
                 **Заполните данные по продуктам**
                1. Введите название каждого ингредиента (например, "Соя", "Шрот").
                2. Укажите содержание протеина в процентах (%). Например, 45.
                3. Введите стоимость продукта за тонну (руб.). Например, 50000.
                Вы можете заполнить от одного до четырех продуктов. Оставьте ненужные поля пустыми или с нулевыми значениями.

                **Настройте добавление карбамида (опционально)**
                1. Поставьте галочку "Использовать карбамид", если он нужен.
                2. С помощью ползунка задайте, какой процент от массы смеси будет занимать карбамид (до 3%).
                3. Укажите цену карбамида за тонну.

                **Задайте параметры конечной смеси**
                1. Введите минимальное требуемое содержание протеина (%) в вашей смеси.
                2. (Опционально) Укажите цену реализации вашей смеси за тонну, чтобы рассчитать прибыль.

                **Запустите расчет**
                1. Нажмите кнопку «Рассчитать».
                2. Результаты появятся ниже. Вы увидите оптимальный состав смеси, ее себестоимость, содержание протеина и расчет прибыли.
                3. При успешном расчете появится кнопка для скачивания результата в TXT формате
                 
                **Сброс и новый расчет**
                Чтобы начать заново, нажмите кнопку «Сбросить».
                 ''')
...
def optimize_protein_mixture_multi(products_data, min_protein, mixture_price=None, use_urea=False, urea_percentage=0.0, urea_price=0.0):
    ...
    if result.success:
        ...
        # Create report for downloading
        report_data = {
            "Параметр": ["Общая стоимость за тонну", "Общий видимый белок", 
                         "Минимальная концентрация протеина", "Цена реализации"],
            "Значение": [f"{optimal_cost:.2f} руб", f"{total_protein:.2f}%", 
                        f"{min_protein:.2f}%", f"{mixture_price:.2f} руб" if mixture_price else "Не указана"]
        }
        
        # Adding product's data
        products_report = []
        for i in range(n_products):
            if optimal_shares[i] > 0:
                products_report.append({
                    "Продукт": products_data[i]['name'],
                    "Доля в смеси": f"{optimal_shares[i] * 100:.2f}%",
                    "Стоимость в смеси": f"{products_data[i]['price_per_kg'] * optimal_shares[i]:.2f} руб",
                    "Содержание протеина": f"{products_data[i]['protein_concentration']}%"
                })
        
        if use_urea and urea_percentage > 0:
            products_report.append({
                "Продукт": "Карбамид",
                "Доля в смеси": f"{urea_percentage:.1f}%",
                "Стоимость в смеси": f"{urea_cost:.2f} руб",
                "Содержание протеина": f"{urea_protein_contribution:.2f}%"
            })
        
        # Создание DataFrame для отчета
        df_report = pd.DataFrame(report_data)
        df_products = pd.DataFrame(products_report)
        
        # Создание текстового отчета
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        report_text = f"Отчет по оптимизации состава смеси\n"
        report_text += f"Сгенерировано: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        report_text += "ОБЩИЕ ПАРАМЕТРЫ:\n"
        
        for _, row in df_report.iterrows():
            report_text += f"{row['Параметр']}: {row['Значение']}\n"
        
        report_text += "\nСОСТАВ СМЕСИ:\n"
        for _, row in df_products.iterrows():
            report_text += f"{row['Продукт']}: {row['Доля в смеси']} (стоимость: {row['Стоимость в смеси']}, протеин: {row['Содержание протеина']})\n"
        
        # Кнопка для скачивания отчета
        st.download_button(
            label="Скачать отчет в формате TXT",
            data=report_text,
            file_name=f"optimization_report_{timestamp}.txt",
            mime="text/plain",
            type="primary"
        )
